...
class YoloV4CameraDetector : public BaseCameraDetector {
 public:
  ...
  bool Detect(const cv::Mat &mat, float thresh, std::vector<std::shared_ptr<bbox_t>> *objects);

  bool Multitask(const cv::Mat &frame, float thresh, std::vector<std::shared_ptr<bbox_t>> *objects);
