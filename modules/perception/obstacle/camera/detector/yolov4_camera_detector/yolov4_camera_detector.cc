...
bool YoloV4CameraDetector::Init(const CameraDetectorInitOptions &options) {
  // load yolo camera detector config file to proto
  CHECK(GetProtoFromFile(FLAGS_yolov4_camera_detector_config, &config_));                                             

  const string &yolo_root = config_.yolo_root();
  const string yolo_config = apollo::common::util::GetAbsolutePath(
      yolo_root, FLAGS_yolov4_config_filename);
...
void YoloV4CameraDetector::init_anchor(const string &yolo_root) {
...
void YoloV4CameraDetector::load_nms_params() {
...
void YoloV4CameraDetector::load_intrinsic(
  ...
  if (options.intrinsic == nullptr) {
    AWARN << "YoloV4
  CameraDetector options.intrinsic is nullptr. Use default";
    image_height_ = 1080;
    image_width_ = 1920;
  } else {
  ...
  image_data_.reset(
      new caffe::SyncedMemory(image_width * image_height * channel * sizeof(unsigned char)));
}

void YoloV4CameraDetector::load_intrinsic_lane(
    const CameraDetectorInitOptions &options) {
...
bool YoloV4CameraDetector::init_cnn(const string &yolo_root) {
  ...
  AWARN << "init cnn in YOLOV4
CameraDetector";
  AWARN << " proto_file: " << proto_file;
  AWARN << " weight_file: " << weight_file;
  AWARN << " model_root: " << model_root;
  ...
  output_names.push_back(net_param._blob());
...
bool YoloV4CameraDetector::init_cnn_lane(const string &yolo_root) {
  ADEBUG << "yolo_root: " << yolo_root;
...
bool YoloV4CameraDetector::Lanetask(const cv::Mat &frame, cv::Mat *mask) {
  if (mask == nullptr) {
    AERROR << "'mask' is a null pointer.";
    return false;
  }
...
bool YoloV4CameraDetector::Detect(
    const cv::Mat &frame, const CameraDetectorOptions &options,
...
bool YoloV4CameraDetector::get_objects_cpu(
    std::vector<std::shared_ptr<VisualObject>> *objects) {
  int num_classes = types_.size();
  auto scale_1_blob =
      cnnadapter_->get_blob_by_name(yolo_param_.net_param().scale_1_blob());
  auto scale_2_blob =
      cnnadapter_->get_blob_by_name(yolo_param_.net_param().scale_1_blob());
  auto scale_3_blob =
      cnnadapter_->get_blob_by_name(yolo_param_.net_param().scale_1_blob());
...
bool YoloV4CameraDetector::get_objects_gpu(
    std::vector<std::shared_ptr<VisualObject>> *objects) {
  auto loc_blob =
...
void YoloV4CameraDetector::get_object_helper(
